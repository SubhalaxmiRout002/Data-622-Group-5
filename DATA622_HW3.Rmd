...
We load in the data and observe the first 6 observations of our dataset:

```{r}
# Loading & Tidy data 
data <- read.csv("https://raw.githubusercontent.com/SubhalaxmiRout002/Data-622-Group-5/main/Loan_approval.csv", stringsAsFactors = TRUE)
#loan <- data %>% drop_na() #MS: doesn't appear that there are NA values
head(loan)
```

The output above provides some context regarding the format of our data, but we can take this understanding a bit further by utilizing R's built-in glimpse() and summary() functions:
...
We can refer to the output above for insight into the dimensions, variable characteristics, and value range.

We're dealing with a 529 observations x 13 variable dataframe with:
...
* `Loan_ID`, `Gender`, `Married`, `Dependents`, `Education`, `Self_Employed`, `Property_Area`, and `Loan_Status`, all categorical, character-based variables, as independent variables.

From a quick glance a number of these variables appear to be irrelevant for our purposes. As a next step, we’ll visualize, explore further, and identify features that carry promise vs. those that don't.

### LEFT OFF HERE [MS 3.20.21]

We start by exploring the correlation matrix:
...
We start by exploring variable distributions vs. our dependent variable `Loan_Status`:
