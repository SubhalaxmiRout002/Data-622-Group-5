...
This model consisits of all attributes excluding Gender. 
...
More features better for the model, we will exclude Gender variable because logically it is not correct that for Female loan approval will be less and more for Male. Most of data having zero dependants, so exclude Dependents. 
...
Helps with feature selection based on importance and avoids over-fitting.
...
Create first model with all variables except gender and Dependents.
...
We will do the model evaluation, apply RF model 1 and model 2 with test dataset to see the accuracy and other matrices. 
...
We have an accuracy of 80 % for first model and 81.82 %for second model.
...
```{r}
#Pre-process data
#1. normalize independent numeric vars
...
loan_GB <- subset(loan, select = -c(Gender)) #why drop Gender?
...
if(loan_GB$Property_Area=="Semiurban")
  {
    loan_GB$Property_Area <- 2
} else if(loan_GB$Property_Area=="Urban"){
    loan_GB$Property_Area <- 1
} else {
    loan_GB$Property_Area <- 0 #rural
}
...
dim(train_GB) #457 x 11
dim(test_GB) #157 x 11
```

```{r}
#Create train, test matrices - one hot encoding for factor variables
library(Matrix)
trainm <- sparse.model.matrix(Loan_Status ~ ., data = train_GB) # rmv Loan_Status col (?)
head(trainm)
train_label <- train_GB[,"Loan_Status"]
#train_label <- train_label - 1 #to account for num_class error
...
GB_model <- xgb.train(params = xgb_params,
                      data = train_matrix,
                      nrounds = 100, #run 100 iterations
                      watchlist = watchlist)

###MS LEFT OFF HERE: 4.6.21
```

The above code snippet converts factor variables into "dummie variables". In our case, that's exclusively `Loan_Status`.
...
```{r}
# Using the Gradient Boosting to predict the loan approval status
# loan.boost=gbm(Loan_Status ~ . ,
#                data = train,
...
# loan.boost
# 
# ntree_opt_cv <- gbm.perf(loan.boost, method = "cv")
```
...
Using its internal calculations such as Variable Importance, it will also point whether any of the predictor variables had zero influence in the model.The above Gradient Boosted Model which generates 10000 trees and the shrinkage parameter *lambda = 0.01* which is also a sort of learning rate.Summary of the plot gives us the feature importance plot. This table ranks the individual variables in the loan dataset based on their relative influence. On the top of the summary list are the two most important features are *Applicationincome* and *LoanAmount*.The gbm package comes with a default function called gbm.perf() to determine the optimum number of iterations. Our model will stop the training procedure on the given optimum number of iterations.  

### Predictions using gbm

We are using the prediction() function to generate the predictions out of the data. Since we have figured out the optimum number of trees by performing early stopping 

```{r gradientboostplot2}
#predictions <- predict(object = loan.boost,
#                       newdata = test,
#                       n_trees = ntree_opt_cv,
#                       type = "response")
#print(predictions)
```

Now let's assess the predictions and guage the model's performance. First we will create a confusion matrix. Confusion matrix will compare the model's actual values with predicted values. 

```{r}
#caret::confusionMatrix(
#  data = factor(predictions, levels = 1:614),
#  reference = factor(test$Loan_Status, levels = 1:614)
#  )
```
