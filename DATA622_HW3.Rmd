...
Before create model lets see size of loan dataset and check for data is balanced or unbalanced
...
Data is not imbalanced, we can proceed with Decision Trees. 
...
```{r}
# Miscalssification Error with Train data
tab <- table(Predicted = predict(tree), Actual = train_data$Loan_Status)
print(tab)
print(paste('Miscalssification Error with Train data', round(1 - sum(diag(tab))/sum(tab),3)))

# Miscalssification Error with Test data
testPred <- predict(tree, test_data)
testtab <- table(Predicted = testPred, Actual = test_data$Loan_Status)
print(testtab)
print(paste('Miscalssification Error with Test data', round(1 - sum(diag(testtab))/sum(testtab),3)))
...
train <- subset(loan, sample_data == TRUE)
test <- subset(loan, sample_data == FALSE)
```
...
#confusionMatrix(PredictionsBinaries, test$Loan_Status)
```
