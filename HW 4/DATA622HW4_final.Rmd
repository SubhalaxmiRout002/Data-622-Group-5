...
* `ADHD.Tot` has the most weight for `PC1` and 
...
### SVM Model: entire dataset and PCA output
...
The models share the same Accuracy and Neg Pred Value, whereas  SVM_Model_3 (radial) outperformed SVM_Model_4 (linear) in Sensitivity, Recall, Detection Ratee, F1, and Detection Prevalence while SVM_Model_4 (linear) outperformed SVM_Model_3 (radial) for Specificity, Pos Pred Value, Precision, and Balanced Accuracy. 
...
* Model 5 (Radial) has high accuracy, Precision, and F1 than Model 6 (linear). 
* Model 5 works fine with predicting 'no' but have  higher class error in predicting ‘yes’. Similarly, Model 6 correctly predict for 'no' but completely incorrectly predict for 'yes'. Incorrect prediction on 'yes' class may be the cause of Multicollinearity.  
