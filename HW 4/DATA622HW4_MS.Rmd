...
* `PC1` is strongly (positively) correlated with `ADHD.Total`,
...
First we need to identify NAs using `vis_miss` from visdat package. Then replace NAs with Mode. 
...
This is little suprising, non Disorderly conduct committed more suicide than Disorderly Conduct.
...
### SVM Model using entire dataset and PCA output
...
svm3 <- predict(tune.out_radial$best.model, test_all[,-dim(test_all)[2]])
svm4 <- predict(tune.out_linear$best.model, test_all[,-dim(test_all)[2]])

#Convert performance statistics to tabular form (for output / interpretation):
AccuracySVM3 <- confusionMatrix(svm3, as.factor(results))$overall['Accuracy']
AccuracySVM4 <- confusionMatrix(svm4, as.factor(results))$overall['Accuracy']

SVM_Model_3 <- confusionMatrix(svm3, as.factor(results))$byClass
...
tabularview <- data.frame(SVM_Model_3, SVM_Model_4)
tabularview %>%  kableExtra::kbl() %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"),latex_options="scale_down")
```

The models share the same Accuracy and Neg Pred Value, whereas  SVM_Model_1 (radial) outperformed SVM_Model_2 (linear) in Sensitivity, Recall, Detection Ratee, F1, and Detection Prevalence while SVM_Model_2 (linear) outperformed SVM_Model_1 (radial) for Specificity, Pos Pred Value, Precision, and Balanced Accuracy. 

#### SVM Model building using PCA

From `Varimax` plot we found, to get above 90% variance in data we need to include 11 principal componenets. We are going to exclude 'ADHD.Total','Non.subst.Dx', 'Subst.Dx', from the dataset.
...
tune.out_radial_5 <- tune.svm(as.factor(Suicide) ~., data=training_not_all, 
                 kernel='radial', cost=2^(1:5), gamma = gammalist)

tune.out_linear_6 <- tune.svm(as.factor(Suicide) ~., data=training_not_all, 
                 kernel='linear', cost=2^(1:5), gamma = gammalist)

svm5 <- predict(tune.out_radial_5$best.model, test_not_all[,-dim(test_not_all)[2]])
svm6 <- predict(tune.out_linear_6$best.model, test_not_all[,-dim(test_not_all)[2]])

#Convert performance statistics to tabular form (for output / interpretation):
AccuracySVM5 <- confusionMatrix(svm5, as.factor(results_not_all))$overall['Accuracy']
AccuracySVM6 <- confusionMatrix(svm6, as.factor(results_not_all))$overall['Accuracy']

SVM_Model_5 <- confusionMatrix(svm5, as.factor(results_not_all))$byClass
SVM_Model_5 <- data.frame(SVM_Model_5)
SVM_Model_5 <- rbind("Accuracy" = AccuracySVM5, SVM_Model_5)

SVM_Model_6 <- confusionMatrix(svm6, as.factor(results_not_all))$byClass
SVM_Model_6 <- data.frame(SVM_Model_6)
SVM_Model_6 <- rbind("Accuracy" = AccuracySVM6, SVM_Model_6)

tabularview <- data.frame(SVM_Model_3, SVM_Model_4, SVM_Model_5, SVM_Model_6)
tabularview %>%  kableExtra::kbl() %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"),latex_options="scale_down")
```

Model 5 (Radial) has high accuracy, Pricision, and F1 than Model 6 (linear). 
Model 5 works fine with predicting 'no' but have  higher class error in predicting ‘yes’. Similaryl, Model 6 correctly predict for 'no' but completely incorectly predict for 'yes'. Incorect prediction on 'yes' class may be the cause of Multicollinearity.  
Model 3 and Model 4 are the better models than Model 5 and Model 6 aspect of accuracy, Specificity, F1.
