...
# created 3 datasets for LR, Neural nets, NN using keras
loan_new2 <- loan
loan_nn <- loan
loan_keras <- loan
```
...
This model is a densely connected deep neural network with two hidden layers and an output layer. Training a neural network revolves around the following objects:
...
The above model has 5 input layers, 2 hidden layers (i.e 4 nurons in one layer and other has 2 neurons). This is connect to the output neuron. We make the dataset to a matrix format, and split in to train and test. 
...
Split the data in to train and test, train data 80% and test data 20%. Create 4 sets of data i.e training, testing, train traget and test target. Train and test traget contains the ouput variable i.e Loan Status. Training and testing datasets contains all other variables other than output variable.
...
Normalize the values apply mean and standard deviation to the columns. All the values present in between 0 and 1 in training and testing set. 
...
The first graph is for loss and 2nd graph is for mean absolute error. Initially there are very less difference between loss and val_loss which is very minimal. This means the training error is low. But after epochs 14 the error is gradually increasing. We see similar pattern in 2nd graph.
...
We have used (4:2) neurons and only 2 hidden layer, there are scope for improvement. We will try with multiple neurons and layer dropout that helps to reduce over fitting. 
...
```{r}
#Fit model
model_keras_train <- model_keras_2 %>%
         fit(training_keras,
             trainingtarget,
             epochs = 100,
             batch_size = 32,
...
Accuracy improved 78% to 80%, this model also perform same as first keras model. Better performance with Yes category and poor performance with No category. There are total 22 misclassification. 
